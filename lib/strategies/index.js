...
async function laborious(ctx, {
  ...
  for (let attempt = 1; attempt <= maxTry && !(successfulTitle && successfulArtist); attempt++) {
    const expectedDelay = (Math.random() * (maxDelay - minDelay)) + delay.min;
    ...
    const delay = elapsed > expectedDelay ? elapsed - expectedDelay : 0;
...
function displayResponse(track, options) {
  ...
  response = _.lowerCase(_.deburr(_.replace(_.join(response, ' '), /[^ \w]/g, '')));
